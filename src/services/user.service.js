...
const ensureRoleExists = async (vaiTroId) => {
  ...
  if (!role) {
    throw new AppError('vai_tro_id không tồn tại', 400);
  }
...
const ensureNotDisableLastActiveAdmin = async ({ targetUser, nextRoleName, nextStatus }) => {
  ...
  if (otherActiveAdmins <= 0) {
    throw new AppError('Không thể vô hiệu hóa admin active cuối cùng', 400);
  }
};
...
const updateMyProfile = async (nguoiDungId, payload, context = {}) => {
  ...
  await writeAuditLog({
    ...
    ghi_chu: 'Cập nhật hồ sơ cá nhân',
...
const createUser = async (actor, payload, context = {}) => {
  ...
  await writeAuditLog({
    ...
    ghi_chu: `Admin tao tài khoản ${createdUser.ten_dang_nhap}`,
    ip_address: context.ipAddress,
  });
...
const updateUser = async (actor, nguoiDungId, payload, context = {}) => {
  ...
  await writeAuditLog({
    ...
    du_lieu_cu: toPublicUser(currentUser),
    du_lieu_moi: toPublicUser(updatedUser),
    ghi_chu: `Admin cap nhat tài khoản ${updatedUser.ten_dang_nhap}`,
    ip_address: context.ipAddress,
  });
...
const updateUserStatus = async (actor, nguoiDungId, payload, context = {}) => {
  ...
  await writeAuditLog({
    ...
    ghi_chu: payload.ly_do || 'Cập nhật trạng thái tài khoản',
    ip_address: context.ipAddress,
  });
...
const resetUserPassword = async (actor, nguoiDungId, payload, context = {}) => {
  ...
  if (isSamePassword) {
    throw new AppError('mat_khau_moi không được trùng với mat_khau hiện tại', 400);
  }
  ...
  await writeAuditLog({
    ...
    entity_name: 'nguoi_dung',
    entity_id: nguoiDungId,
    ghi_chu: `Admin reset mật khẩu cho tài khoản ${targetUser.ten_dang_nhap}`,
    ip_address: context.ipAddress,
  });
