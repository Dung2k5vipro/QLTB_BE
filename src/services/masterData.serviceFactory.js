...
const createMasterDataService = ({
  ...
  const ensureUniqueFields = async (payload, excludeId = null) => {
    for (const field of uniqueFields) {
      ...
      if (exists) {
        throw new AppError(`${field} đã tồn tại`, 409);
      }
    }
  };
  ...
  const mapDatabaseError = (error) => {
    ...
    if (duplicatedField) {
      return new AppError(`${duplicatedField} đã tồn tại`, 409);
    }
  ...
  const create = async (actor, payload, context = {}) => {
    try {
      ...
      await writeAuditLog({
        ...
        ghi_chu: `Tạo moi ${entityName}${createdItem?.[displayField] ? `: ${createdItem[displayField]}` : ''}`,
  ...
  const update = async (actor, id, payload, context = {}) => {
    ...
    await writeAuditLog({
      ...
      ghi_chu: `Cập nhật ${entityName}${updatedItem?.[displayField] ? `: ${updatedItem[displayField]}` : ''}`,
      ip_address: context.ipAddress,
    });
  ...
  const updateStatus = async (actor, id, payload, context = {}) => {
    ...
    await writeAuditLog({
      ...
      ghi_chu: `Chuyen trạng thái ${entityName} sang ${nextStatus ? 'active' : 'inactive'}`,
      ip_address: context.ipAddress,
    });
