...
const createUser = {
  body: (body) => {
    ...
    if (mat_khau.length < MIN_PASSWORD_LENGTH) {
      throw new AppError(`mat_khau phai co t?i thi?u ${MIN_PASSWORD_LENGTH} ky tu`, 400);
    }
...
const resetUserPassword = {
  body: (body) => {
    ...
    if (mat_khau_moi.length < MIN_PASSWORD_LENGTH) {
      throw new AppError(`mat_khau_moi phai co t?i thi?u ${MIN_PASSWORD_LENGTH} ky tu`, 400);
    }
